...
<body class="gray-bg">
    <div class=""><a class="btn btn-info" href="{{url('/articleCategory/create')}}" target="iframe6" >添加分类</a></div>
                    ...
                    <div class="ibox-content">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$category->id}}</td>
                                    <td>{{$category->newHtml}}</td>
                                    <td>{{$category->parent_id}}</td>
                                    <td>{{$category->created_at}}</td>
                                    <td>
                                        <a class="btn btn-info" href="{{url('articleCategory/'.$category->id.'/edit')}}" target="iframe6">修改</a>
                                        <form action="{{url('articleCategory/'.$category->id)}}" method="post" target="iframe6" style="display: inline-block">
                                            {{ method_field('DELETE') }}     
                                            {{ csrf_field() }}                
