...
<body class="gray-bg">
    <div class=""><a class="btn btn-info" href="{{url('/tags/create')}}" target="iframe7" >添加标签</a></div>
                        ...
                        <table class="table table-bordered">
                            ...
                            @foreach($tags as $tag)
                                <tr>
                                    ...
                                    <td>{{$tag->tag}}</td>
                                    <td>
                                        <a href="{{url('tags/'.$tag->id.'/edit')}}" class="btn btn-info" target="iframe7">修改</a>
                                        <form action="{{url('tags/'.$tag->id)}}" method="post" target="iframe7" style="display: inline-block">
                                            <!--删除时要使用DELETE方法-->
                                            {!! csrf_field() !!}
